...
def get_cnnmodel(class_num=2,tuner_kernel_size=[13,14,15],lr=0.001,dropout=0.1):
    ...
    kernel_sizes=tuner_kernel_size
    convs = []
    max_poolings = []
...
param_grid = {
    ...
    'tuner_kernel_size': [[13,14,15],[10,12,14],[13,15],[10,15]],
...
params={"lr":0.001,'tuner_kernel_size': [13,14,15],'batch_size':64,'dropout':0.1}
...
for train_index, test_index in outer_kf.split(X,y,groups=df["id"].values.tolist()):
    ...
    for params in param_grid:
        ...
        for inner_train_idx, inner_test_idx in inner_kf.split(X_train,y_train):
            ...
            model=get_cnnmodel(tuner_kernel_size=params['tuner_kernel_size'],lr=params['lr'],dropout=params['dropout'])
            ...
            prediction=np.argmax(model.predict([X_test_inner]),axis=1)
